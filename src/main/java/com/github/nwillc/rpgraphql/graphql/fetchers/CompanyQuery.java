...
public class CompanyQuery extends CompanyFetcher<Company> {
    public CompanyQuery(Map<String, Company> companies) {
        super(companies);
    }

    @Override
    public Company get(DataFetchingEnvironment environment) {
        return getCompanies().get(environment.getArgument("name"));
    }
}
